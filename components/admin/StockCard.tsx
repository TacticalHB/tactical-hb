...
const TONE: Record<string, { bg: string; fg: string }> = {
  critical: { bg: "#f9e9e8", fg: "#96322c" },
  low: { bg: "#fbf1de", fg: "#8a5d16" },
  ok: { bg: "#e7f2ec", fg: "#2f6b4f" },
};
  ...
  return (
    <article className="rounded-lg" style={{ border: "1px solid var(--border)", background: "#fff" }}>
      <header
        ...
        style={{ borderBottom: open ? "1px solid var(--border)" : "none", background: "#fcfcfb" }}
      >
        <span className="text-[14.5px] font-medium" style={{ color: "#111" }}>
          {itemName(item, uk)}
        </span>
        {item.kind === "part" && (
        ...
        <span className="font-mono text-[11.5px]" style={{ color: "#a3a3a6" }}>
          {item.sku}
        </span>

        <span className="ml-auto text-[12px]" style={{ color: "#8a8a8d" }}>
          {L.unitCost}:{" "}
          {item.unitCostUah === null ? (
            <span style={{ color: "#c0bfbc" }}>{L.noCost}</span>
          ) : (
        ...
        <span className="text-[19px] font-medium tabular-nums w-[3.5ch] text-right" style={{ color: "#111" }}>
          {onHand}
        </span>
        ...
        <div className="px-5 py-4 grid gap-6 md:grid-cols-3">
          ...
          <form onSubmit={onReceive}>
            ...
            <div className="flex items-center gap-2 mb-2">
              <input
                ...
                inputMode="numeric"
                autoComplete="off"
                className="h-9 px-3 text-[13px] rounded w-[90px] outline-none transition-colors focus:border-black"
                style={inputStyle}
              />
              <button
                type="submit"
            ...
            <input
              ...
              placeholder={L.note}
              autoComplete="off"
              className="h-9 px-3 text-[13px] rounded w-full outline-none transition-colors focus:border-black"
              style={inputStyle}
            />
          </form>
            ...
            <div className="flex items-center gap-2">
              <div>
                <input
                  ...
                  className="h-9 px-3 text-[13px] rounded w-[70px] outline-none transition-colors focus:border-black"
                  style={inputStyle}
                />
                <div className="text-[10.5px] mt-1" style={{ color: "#a3a3a6" }}>{L.critical}</div>
              </div>
              <div>
                <input
                  value={reorder}
                  ...
                  style={inputStyle}
                />
                <div className="text-[10.5px] mt-1" style={{ color: "#a3a3a6" }}>{L.reorder}</div>
              </div>
              <button
                type="submit"
                disabled={busy !== null}
          ...
          <div className="md:col-span-3">
            ...
            {item.movements.length === 0 ? (
              <p className="text-[13px]" style={{ color: "#a3a3a6" }}>{L.noHistory}</p>
            ) : (
                ...
                {item.movements.map((m) => (
                  <li key={m.id} className="flex flex-wrap items-baseline gap-x-3 text-[12.5px]">
                    <span
                      ...
                      style={{ color: m.delta < 0 ? "#96322c" : "#2f6b4f" }}
                    >
                      {m.delta > 0 ? `+${m.delta}` : m.delta}
                    </span>
                    <span style={{ color: "#4a4a4d" }}>{reasonLabel(m.reason, uk)}</span>
                    {m.note && <span style={{ color: "#8a8a8d" }}>{m.note}</span>}
                    <span className="ml-auto" style={{ color: "#a3a3a6" }}>
          ...
          {(error || done) && (
            <div className="md:col-span-3 text-[12.5px]" style={{ color: error ? "#b3261e" : "#4a7c59" }}>
              {error ?? done}
            </div>
          )}
        </div>

...
export default function FavouritesMenu({ locale }: { locale: string }) {
  ...
  // Shared state: guest (localStorage) or logged-in (Supabase) — same API.
  ...
  const L = {
    ...
    viewAll: uk ? "Переглянути всі" : "View all",
  };
      ...
      {open && (
        ...
        >
          <div className="px-4 py-3 flex items-center justify-between border-b" style={{ borderColor: "var(--border)" }}>
            <span className="text-sm font-semibold" style={{ color: "var(--ink)" }}>{L.title}</span>
            <span className="text-xs tabular-nums" style={{ color: "var(--text-muted)" }}>{items.length}</span>
          </div>
          ...
          {items.length === 0 ? (
            ...
            <>
              <ul className="max-h-[360px] overflow-y-auto">
                {items.map((p) => (
                  <li key={p.slug} data-fav-row className="flex items-center gap-3 px-4 py-3 border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                    <Link href={`/${locale}/products/${p.slug}`} onClick={() => setOpen(false)}
                      className="relative w-14 h-14 shrink-0 rounded-md overflow-hidden" style={{ background: "#f5f5f5" }}>
                      <Image src={p.gridImage || p.image} alt="" fill sizes="56px" className="object-contain" />
                    </Link>
                    <div className="min-w-0 flex-1">
                      <Link href={`/${locale}/products/${p.slug}`} onClick={() => setOpen(false)}
                        className="block text-[13px] font-medium truncate hover:opacity-70" style={{ color: "var(--ink)" }}>
                        {uk ? p.nameUk : p.nameEn}
                      </Link>
                      <div className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>€{p.price.toFixed(2)}</div>
                      <button
                        onClick={(e) => {
                    ...
                    <button onClick={() => toggleFavourite(p.slug)} className="shrink-0 p-1 hover:opacity-60"
                      aria-label={L.remove} style={{ color: "var(--text-faint)" }}>
              ...
              <Link href={`/${locale}/account/favourites`} onClick={() => setOpen(false)}
                className="block text-center text-xs py-3 border-t hover:bg-[color:var(--bg-soft)]"

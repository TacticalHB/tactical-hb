...
type VoucherRow = { code: string; amount_eur: number; min_order_eur: number; status: string; expires_at: string };
...
export default function LoyaltyDashboard({
  ...
  const L = {
    ...
    noVouchers: uk ? "Ще немає ваучерів — витрачайте, щоб відкрити." : "No vouchers yet — spend to unlock your first.",
    active: uk ? "Активний" : "Active",
    expired: uk ? "Прострочений" : "Expired",
    used: uk ? "Використаний" : "Redeemed",
  ...
  const voucherStatus = (v: VoucherRow) => {
    if (v.status === "redeemed") return { label: L.used, ok: false };
    if (new Date(v.expires_at).getTime() < now || v.status === "expired") return { label: L.expired, ok: false };
    return { label: L.active, ok: true };
  };
      ...
      {vouchers.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {vouchers.map((v) => {
            ...
            return (
              <div key={v.code} className="rounded-2xl border p-5 flex items-center justify-between gap-4"
                style={{ borderColor: "var(--border)", opacity: st.ok ? 1 : 0.55 }}>
                <div>
                  <div className="text-2xl font-semibold" style={{ color: "#111" }}>{money(v.amount_eur)}</div>
                  <div className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>{L.minOrder(money(v.min_order_eur))}</div>
                  <div className="text-xs mt-2 font-mono tracking-wider" style={{ color: "#111" }}>{v.code}</div>
                  <div className="text-xs mt-1" style={{ color: "var(--text-faint)" }}>{L.expires} {dateFmt(v.expires_at)}</div>
                </div>
                <span className="text-[11px] uppercase tracking-wide px-2.5 py-1 rounded-full shrink-0"
                  style={{ background: st.ok ? "#f8f880" : "var(--bg-soft)", color: st.ok ? "#111" : "var(--text-muted)" }}>
                  {st.label}
                </span>
              </div>
            );
